...
def test_cli_input_file(monkeypatch, tmp_path, capsys):
    ...
    write_to = tmp_path / "test_lazytex_output.md"

    monkeypatch.setattr("builtins.input", lambda _: "y")
    test_args = parse_args([str(input_file), "-w", str(write_to)])
    exit_code = main(test_args)
    assert write_to.is_file()
    assert exit_code == 0
    # Delete the output file so that it does not interfere with other tests
    write_to.unlink()
    assert not write_to.is_file()

    # Test that the program exits when the user does not want to overwrite
    # an existing file
    write_to = Path("tests/test_data/test_dummy_out.txt")
    ...
    test_args = parse_args([str(input_file), "-a", str(write_to)])
    ...
    # append to
    write_to = tmp_path / "test_non_existing_write.md"
    test_args = parse_args([str(input_file), "-a", str(write_to)])
    with pytest.raises(SystemExit) as exc_info:
        main(test_args)
    assert exc_info.type == SystemExit
...
def test_invalid_args(capsys, tmp_path):
    ...
    write_to = tmp_path / "test_lazytex_output.md"
    test_args = parse_args([str(invalid_input_file), "-w", str(write_to)])
...
def test_cli_statement_arg(monkeypatch, capsys):
    ...
    or append flag. Overwrites a file if the user adds the -w flag, or appends
    ...
    test_args = parse_args(
        ["-s", statement, "-w", "tests/test_data/test_dummy_out.txt"])
    main(test_args)
    captured = capsys.readouterr()
    assert captured.out.rstrip() == "Converted statement to LaTeX:\n\n"\
        + expected_latex + \
